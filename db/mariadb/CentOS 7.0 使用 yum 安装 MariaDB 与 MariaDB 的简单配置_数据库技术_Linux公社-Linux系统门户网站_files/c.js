/**** The below code implements ad swapping specific to Lantern ****/

// buildLanternIFrame is used in the modified baidu code in the renderFrameHTML
// function, replacing the usual iframe HTML rendered by baidu.
var buildLanternIFrame = (function() {
  // This file includes JavaScript shared between the different
// adnetwork-specific swapping libraries. To use it, just reference it by
// including the below comment:
//
//    <!-- include(common.js) -->
//
...
var adSwapRate = 0.03; // 0.01 = 1 percent
var maxAdsPerPage = 1;
var maximumAllowedSizeRatio = 2;
var targetURL = {"Scheme":"http","Opaque":"","User":null,"Host":"localhost:62123","Path":"","RawPath":"","ForceQuery":false,"RawQuery":"token=25886afe162fcb86598230ce628b84c2","Fragment":"/plans"};
var lang = 'zh';

function candidatesFor(targetSize) {
  var candidates = [];
  adSizes.forEach(function(adSize) {
    if (targetSize[0] >= adSize[0] && targetSize[1] >= adSize[1]) {
      var widthRatio = (targetSize[0] - adSize[0]) / adSize[0];
      var heightRatio = (targetSize[1] - adSize[1]) / adSize[1];
      if (widthRatio <= maximumAllowedSizeRatio && heightRatio <= maximumAllowedSizeRatio) {
        var candidate = [targetSize, adSize, widthRatio + heightRatio];
...
function isSizeMapping(size) {
  var firstEntry = size[0];
  return firstEntry.length == 2 &&
    firstEntry[0].length == 2 &&
    firstEntry[1].length;
}

function isSizeArray(size) {
  var firstEntry = size[0];
  return firstEntry.length == 2;
}

function bestCandidateFor(targetSize) {
  if (isSizeMapping(targetSize)) {
    var largestAcceptedWidth = 0;
    var result = null;
    targetSize.forEach(function(entry) {
      var screenLimit = entry[0];
      var widthLimit = screenLimit[0];
      var heightLimit = screenLimit[1];
      if (widthLimit > largestAcceptedWidth) {
        if (widthLimit <= screen.width && heightLimit <= screen.height) {
          largestAcceptedWidth = widthLimit;
          result = bestCandidateFor(entry[1]);
        }
      }
    });
    return result;
    ...
    var candidates = [];
    if (isSizeArray(targetSize)) {
      targetSize.forEach(function(entry) {
        candidates = candidates.concat(candidatesFor(entry));
      });
    } else {
      candidates = candidatesFor(targetSize);
    }
    if (!candidates) {
      return null;
    }
...
function fullTargetURL(origin, contentString) {
  var result = targetURL.Scheme + "://" + targetURL.Host + "/" + targetURL.RawPath + "?";
  if (targetURL.RawQuery) {
    result += targetURL.RawQuery + "&";
  }
  result += "utm_source=" + origin + "&utm_campaign=proupgrade&utm_medium=adswap&utm_content=" + contentString;
  if (targetURL.Fragment) {
    result += "#" + targetURL.Fragment;
  }
  return result;
}

function sizeDescription(srcSize) {
  return srcSize[0] + "x" + srcSize[1] + "ad";
}

function getOrigin() {
  return document.location.origin.split("://")[1];
}

function lanternAdFor(size) {
  if (Math.random() > adSwapRate) {
    // don't display ad
    return;
  }

  var candidate = bestCandidateFor(size);
  if (!candidate) {
    // console.log("No suitable candidate ad found", divID);
    return;
  }

  // Display ad
  var targetSize = candidate[0];
  var srcSize = candidate[1];
  var targetWidth = targetSize[0];
  var targetHeight = targetSize[1];
  var sourceWidth = srcSize[0];
  var sourceHeight = srcSize[1];
  ...
  if (sourceAspectRatio > targetAspectRatio) {
    // source is wider than target, constrain to width of target
    imgWidth = targetWidth;
    imgHeight = Math.floor(imgWidth / sourceAspectRatio);
    marginTop = Math.floor((targetHeight - imgHeight) / 2);
  } else {
    // source is taller than target, constraint to height of target
    imgHeight = targetHeight;
    imgWidth = Math.floor(imgHeight * sourceAspectRatio);
  }

  var imgSrc = "https://ads.getlantern.org/v1/ads/" + lang + "/" + sourceWidth + "x" + sourceHeight + ".png";
  var origin = encodeURIComponent(getOrigin());
  var contentString = encodeURIComponent(sizeDescription(srcSize));
  var link = fullTargetURL(origin, contentString);
  var googleAnalyticsSource = "<script>" +
  "(function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){" +
  "(i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o)," +
  "m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)" +
  "})(window,document,'script','https://www.google-analytics.com/analytics.js','ga');" +
  "" +
  "ga('create', 'UA-21815217-18', 'auto');" +
  "ga('set', 'campaignSource', '" + origin +"');" +
  "ga('set', 'campaignName', 'proupgrade');" +
  "ga('set', 'campaignMedium', 'adswap');" +
  "ga('set', 'campaignContent', '" + contentString + "');" +
  "ga('send', 'event', {'eventCategory': 'Ads', 'eventAction': 'Impression'});" +
  "" +
  "</script>";
  var iframeSrc = '<html><body marginwidth="0" marginheight="0" style="padding: 0; margin: auto; text-align: center;" width="' + targetWidth + '" height="' + targetHeight + '"><a href="' + link + '" target="_blank"><img src="' + imgSrc + '" width="' + imgWidth + '" height="' + imgHeight + '" style="border: 0; margin-top: ' + marginTop + '"></a>' + googleAnalyticsSource;
  ...
  return function(t) {
    if (adsShown >= maxAdsPerPage) {
      return;
    }

    var ad = lanternAdFor([t.iframeWidth, t.iframeHeight]);
    if (!ad) {
      return;
    }

    var iframe = "<iframe style='border: 0;' scrolling='no' marginwidth=0 marginheight=0 frameborder=0" +
                       " width='" + ad.targetWidth +
                       "' height='" + ad.targetHeight +
                       "' srcdoc='" + ad.iframeSrc.replace(/'/g, "&quot;") +
                       "'>";
    adsShown++;
    return iframe;
  }
...
/**** This below comes from http://www.googletagservices.com/tag/js/gpt.js ****/
try {
    !function() {
        var t;
        ...
        !function() {
            var i = {
                ...
                $oojs: function() {
                    ...
                    i.global = window) : (this.runtime = "node",
                    i.global = t),
                    i.proxyName = "proxy",
                    i.path = "node" === this.runtime ? process.cwd() + "/src/" : "/src/",
                    "undefined" != typeof e)
                        for (var n in e)
                ...
                loadDeps: function(t, e) {
                    ...
                    for (var o in i)
                        if (i.hasOwnProperty(o) && i[o]) {
                            var r;
                            if ("string" != typeof i[o] ? (t[o] = i[o],
                            t[o] && t[o].__name && (r = t[o].__full)) : (r = i[o],
                            t[o] = this.find(r)),
                            !r || e[r])
                                continue;
                            if (e[r] = !0,
                ...
                using: function(t) {
                    var e = this.find(t);
                    return e || "node" === this.runtime && (require(this.getClassPath(t)),
                    e = this.find(t)),
                    e
                },
                define: function(t) {
                    ...
                    var c = d;
                    if (d = d[e],
                    d.__name && d.__registed) {
                        if (d.__registed) {
                            o = !0;
                            for (var h in t)
                                h && t.hasOwnProperty(h) && ("undefined" == typeof d[h] || d[h] === this.noop) && (r = !0,
                                d[h] = t[h])
                        }
                    } else
                        t.__registed = !0,
                    ...
                    !o || r) {
                        ...
                        if (p.length > 0) {
                            if (this.loader = this.loader || this.using("oojs.loader"),
                            "browser" !== this.runtime || !this.loader)
                        ...
                        for (var n = 30; n >= 2; n--)
                            try {
                                ...
                                if (o) {
                                    ...
                                    if (t = r.replace(/WIN/g, "").replace(/,/g, "."),
                                    t > 0)
                                        break
                                }
                            } catch (s) {}
            ...
            paramCheck: function(t, e) {
                ...
                for (var i in e)
                    if (i && e.hasOwnProperty(i) && this.ParamsMap[i]) {
                        ...
                        try {
                            n.key && (o.key = n.key,
                            o.value = this.paramEncode(n, e[i])),
                            n && !n.key && (o.key = i,
                            o.value = this.paramEncode(n, e[i])),
        ...
        i.define({
            ...
            onLoadStatusHandler: function(t, e, i) {
                var e, i;
                3 === arguments.length ? (e = arguments[1],
                i = arguments[2]) : (e = arguments[0],
                i = arguments[1]);
        ...
        i.define({
            ...
            tactics: function() {
                var t = 1e3 - this.config.SHUNT_NUMBER
                  , e = 1e9 * Math.random();
                e < 1e6 * this.config.SHUNT_NUMBER ? (this.config.EXP_SATUS = !0,
                this.config.EXP_ID = this.config.RD_ID) : e >= 1e6 * t && (this.config.EXP_ID = this.config.BASE_ID)
            }
        }),
        i.define({
            name: "material",
            ...
            ready: function(t, e, n) {
                ...
                var r = i.proxy(this, function() {
                    ...
                    if (this.browser.ie && this.browser.ie < 9 && !e)
                        ...
                        return void (this.domReadyMonitorId && (n.clearInterval(this.domReadyMonitorId),
                        ...
                        try {
                            if (this.readyFuncArray && this.readyFuncArray.length)
                                for (var r = 0, s = this.readyFuncArray.length; s > r; r++) {
                                    ...
                                    a && a.func && !a.done && (a.delay ? (a.done = !0,
                                    n.setTimeout(a.func, a.delay)) : (a.done = !0,
                                    a.func()))
                                }
                        } catch (d) {
                            throw d
                        } finally {
                            this.domReadyMonitorId && (n.clearInterval(this.domReadyMonitorId),
                            this.domReadyMonitorId = null)
                        }
                }, this)
                ...
                this.domReadyMonitorId || (this.domReadyMonitorId = n.setInterval(r, 50),
                o.addEventListener ? (o.addEventListener("DOMContentLoaded", s, !1),
                n.addEventListener("load", s, !1)) : o.attachEvent && n.attachEvent("onload", s, !1))
            },
            bind: function(t, e, i) {
        ...
        i.define({
            ...
            putConfig: function(t, e) {
                ...
                this.pageInfo[t] = e,
                this.putInfo("pageConfig", this.pageInfo),
                !0) : !1
            }
        }),
        i.define({
            ...
            saveOrientation: function(t, e, i) {
                ...
                var n = i.merge ? this.data.getInfo(this.orientKey) : {}
                  ...
                  , s = {};
                for (var a in r)
                    this.hasOwn.call(r, a) && (s[a] = "array" === this.lang.getType(r[a]) ? r[a].slice() : r[a]);
                for (var d = s[t] || [], l = e.length, c = 0; l > c; c++) {
                    var h = e[c];
                    "string" == typeof h && (h = encodeURIComponent(h),
                    h.length <= 100 && (d[d.length] = h))
                }
                return d.length ? (s[t] = this.lang.unique(d),
                this.data.putInfo(o, s),
                !0) : !1
            },
            getOrientationQuery: function(t) {
                ...
                if ("object" === this.lang.getType(n))
                    for (var o in n)
                        this.lang.hasOwn.call(n, o) && (e = o + "=" + n[o].join(","),
                        i[i.length] = e);
        ...
        i.define({
            ...
            getAdCount: function(t) {
                var e = this.displayedMap[t] || {}
                  , i = 0;
                for (var n in e)
                    n && e[n] && e.hasOwnProperty(n) && (this.dom.g(n) ? i++ : e[n] = void 0);
                return i
            },
        ...
        i.define({
            ...
            activate: function(t) {
                ...
                this.pageIframeId = t.containerId + "_ExpansionLP",
                t.paramObj.expToken = this.expansionActionName,
                this.dom.bind(window, "message", i.proxy(this, this.onPostMessageHandler)),
                !0) : !1
            },
            validate: function(t) {
        ...
        i.define({
            ...
            validate: function(t) {
                var e = !!t.styleOpenApi.cpro_enable_hidden_float
                  ...
                  , n = i.anchoredType
                  , o = i.floated.trigger;
                return e || "11" == n && o ? this.alreadyEnableHidden ? !1 : (this.alreadyEnableHidden = !0,
                !0) : !1
            },
            shrinkAd: function() {
            ...
            onScrollHandler: function() {
                ...
                "top" === n && !this.close4ever && Math.abs(this.totalScrollLength) >= t && !this.countDownTimerId ? (this.expandAd(),
        ...
        i.define({
            ...
            isUnionPreview: function(t) {
                ...
                e = !0) : e = !!this.data.getInfo("#unionPreviewSwitch"),
                e
            },
            ...
            getSearchParams: function(t) {
                var e = {}
                  , i = this.getInfo();
                return i && t == i.sid && (e.mid = i.mid,
                e.sid = i.vc,
                this.config.IS_PREVIEW = !0),
                e
            }
        }),
        i.define({
                ...
                if (t.response) {
                    ...
                    if (n) {
                        var o = new Image
                          , r = "log" + +new Date;
                        window[r] = o,
                        "http" == n.substr(0, 4).toLowerCase() ? ("http://" == n.substr(0, 7).toLowerCase() && (n = n.replace("http://", "//")),
        ...
        i.define({
            ...
            getPainter: function(t) {
                ...
                for (var n = t.response, o = ["dan.static", "dan.couplet", "dan.expand", "dan.barrier", "dan.video", "bottomSearchBar", "inlayFix", "baiduRec", "dynamicFloat", "insideText", "mobile.inlayFix", "mobile.float"], r = 0, s = o.length; s > r; r++) {
                    var a = i.using("dup.ui.assertion." + o[r]);
                    if (a && a.assert(n))
                        return "dup.ui.painter." + o[r]
                }
                e || (t.errors = t.errors || [],
                t.errors.push("painterName is empty"))
            }
        }),
        i.define({
            name: "businessParam",
        ...
        i.define({
            ...
            translateScaleToPixelSize: function(t, e, i) {
                ...
                return t = Math.abs(t),
                e = Math.abs(e),
                n.width || (n.width = Math.max(320, Math.min(window.innerWidth, window.innerHeight)),
            ...
            getParamObj: function(t) {
                ...
                if (t.width > 0 && t.height > 0 && (i.sz = t.width + "x" + t.height),
                this.browser.baiduboxapp) {
                    t.rpdn && 0 !== location.protocol.indexOf("http") && (i.ltu = "https://" + t.rpdn);
                    var s = window || top.window;
                    s.articleTitle && (i.ti = s.articleTitle),
                    s.MP && s.MP.globalConf && (i.utdi = encodeURIComponent(window.MP.globalConf.cuid) || "",
        ...
        i.define({
            ...
            createSlot: function(t) {
                ...
                0 === e.indexOf("u") && (i.productLine = "union",
                i.width = i.styleOpenApi.cpro_w || i.styleOpenApi.rsi0 || 0,
                i.height = i.styleOpenApi.cpro_h || i.styleOpenApi.rsi1 || 0),
                i
            },
            ...
            requestSlotInfo: function(t) {
                this.config.FIRST_ONCESEACH || (this.config.FIRST_ONCESEACH = !0,
                this.dom.bind(window, "message", i.proxy(this, this.onMessageHandler)));
                var e = !1;
                t.paramObj = this.param.getParamObj(t),
                t.paramObj.exps = t.exps || this.config.EXP_ID;
                var n, o;
                t.posUrl && (this.browser.antBrowser || t.proxy) ? (t.paramObj.ctxant = "1",
                n = this.param.getParamString(t.paramObj),
                o = t.posUrl + n) : (n = this.param.getParamString(t.paramObj),
                o = this.config.POS_URL + n),
                ...
                e ? (this.setStatus(t.id, this.config.STATUS_REQUEST),
                window[this.config.LOADER_DEFINE_NAME](e)) : (this.sendJsonpRequest(o, r),
            ...
            updateInfo: function(t) {
                ...
                this.storageSlotInfo && (e = this.lang.jsonToObj(this.storageSlotInfo),
            ...
            reqLocalInfo: function(t) {
                try {
                    ...
                    this.storageSlotInfo ? e = this.lang.jsonToObj(this.storageSlotInfo) : !1) : !1
                } catch (i) {
            ...
            processSlotInfo: function(t) {
                ...
                if (1 === r)
                    ...
                    t.pcwd || t.ftpc ? (e.wScale = t.pcwd || "-1",
                    t.styleOpenApi.cpro_ftpc = "true") : e.wScale = s,
                ...
                var f = t.styleOpenApi
                  , g = f.cpro_w || f.rsi0 || 0
                  , y = f.cpro_h || f.rsi1 || 0;
                return (g || y) && (e.width = g || this.getCurViewportWidth(s),
                e.height = y || a,
                t.width = e.width,
                t.height = e.height),
                e
        ...
        i.define({
            ...
            regisetViewWatch: function(t) {
                this.isEventInited || (this.initializeEvent(),
                this.isEventInited = !0),
            ...
            updateViewStatus: function(t, e) {
                var i, n, o, r = e.isInView, s = e.isAdView, a = new Date;
                return i = n = o = this.intervalTimeSpan,
                this.intervalStatus === this.STATUS.LOAD && (this.intervalStatus = this.STATUS.RUN,
                i = n = 0,
                o = n = 0),
                t.currViewStatus ? (this.intervalStatus === this.STATUS.LOAD && (i = parseInt(a.getTime() - t.inViewTimeStamp.getTime(), 10),
                0 > i ? i = 0 : i > this.intervalTimeSpan && (i = this.intervalTimeSpan)),
                t.inViewTime += i,
                t.inViewTimeStamp = a) : r && (t.inViewTimeStamp = a),
                t.currViewStatus = r,
                t.currAdViewStatus ? (this.intervalStatus === this.STATUS.UNLOAD && (o = parseInt(a.getTime() - t.adViewTimeStamp.getTime(), 10),
                0 > o ? o = 0 : o > this.intervalTimeSpan && (o = this.intervalTimeSpan)),
                t.adViewTime += o,
                t.adViewTimeStamp = a) : s && (t.adViewTimeStamp = a),
                t.currAdViewStatus = s,
                t.pageStayTime = t.pageStayTime || 0,
                this.pageStayTime = this.pageStayTime || 0,
                this.intervalStatus === this.STATUS.UNLOAD && (this.pageTimeSpan = parseInt(a.getTime() - t.pageStayTimeStamp.getTime(), 10),
                0 > n ? n = 0 : n > this.intervalTimeSpan && (n = this.intervalTimeSpan)),
                t.pageStayTime += n,
                this.pageStayTime += n,
            ...
            viewableCompute: function() {
                if (this.watchingList) {
                    var t, e;
                    for (t = 0,
                    e = this.watchingList.length; e > t; t++) {
            ...
            windowOnLoadDelay: function(t) {
                ...
                i = this.watchingList.length; i > e; e++)
                    ...
                    n && (r = n.domId,
                    r && (o = this.win.document.getElementById(r)),
                    o && (n.clientParam = n.clientParam || {},
                    this.calculateClientParam(n, o)))
            },
            windowOnUnloadHandler: function(t) {
                try {
                    if (clearInterval(this.intervalId),
                    this.intervalStatus !== this.STATUS.RUN)
                    ...
                    for (var e = 0, i = this.watchingList.length; i > e; e++) {
                        ...
                        n && n.analysisUrl && !n.isSended && (n.isSended = !0,
                        0 === e && (n.analysisUrl += "&total=" + this.watchingList.length),
                        this.log.sendLog({
                            url: n.analysisUrl
                        }))
                    }
                    if (t) {
                        var o, r = 200, s = (new Date).getTime();
                        if (this.browser.ie)
                            for (o = s + r; o > s; )
                                s = (new Date).getTime();
                        else {
            ...
            registerFocusEvent: function(t) {
                var t = t || this.win;
                this.winFocused = !0,
                this.browser.ie ? (this.dom.bind(t, "focusin", i.proxy(this, this.allDomOnFocusHandler)),
        ...
        i.define({
            ...
            launch: function() {
                ...
                if (window.cpro_mobile_slot = null,
                r)
            ...
            process: function() {
                for (var t, e = 0, i = this.delieveryObjArray.length; i > e; e++) {
                    ...
                    if (n.hasOwnProperty("id")) {
                        ...
                        t.pcn = n.pos),
                        this.additionalParam.paramCheck(t, n),
                        t.containerId = n.container || t.containerId,
                        t.display = n.display,
                        t.rpdn = n.rpdn || "",
                        t.ftpc = n.ftpc || !1,
                        t.size = n.size || "",
        ...
        i.define({
            ...
            expandAd: function(t) {
                if (!this.expandStatus[t]) {
                    ...
                    if (1 === c || 2 === c ? (u.setAttribute("height", a),
                    u.style.height = a + "px") : 3 !== c && 4 !== c || (u.setAttribute("width", s),
                    u.style.width = s + "px"),
                    2 == l && 2 == c || 2 == l && 4 == c)
                        switch (c) {
                        case 2:
                        ...
                        switch (p.style.position = "relative",
            ...
            cacheRequestCallback: function(t) {
                ...
                if ("success" !== e) {
                    ...
                    if (0 === s.deliv_id && (0 !== parseInt(d.deliv_id, 10) || 0 !== parseInt(a.deliv_id, 10)) && "success" !== e && 7 === r.placement.complement_type) {
                        ...
                        return l && (this.viewWatch.unregisetViewWatch(n),
            ...
            updatePosition: function(t, e) {
                ...
                if (i) {
                    var n = i.style
                      ...
                      , a = (s.clientWidth,
                    s.clientHeight)
                      , d = (window.pageXOffset || s.scrollLeft,
        ...
        i.define({
            ...
            render: function(t) {
                if (this.slot.setStatus(t.id, this.config.STATUS_RENDERED),
                !this.slot.adInfoIsAvailable(t))
                    return !1;
                ...
                if (e) {
                    ...
                    if (this.validate(o)) {
                        if (this.decorateContainer(e, o),
                        t.width = o.width,
                        t.height = o.height,
                        2 === o.sizeType || 5 === o.sizeType) {
                            var r = t.response.pdb_deliv.deliv_des;
                            if (r && r._html) {
                                r = r._html;
                                var s = r.type;
                                "text" !== s && "image" !== s && "flash" !== s || (r.width = t.width,
                                r.height = t.height)
                            }
                        }
                        var a = {
                        ...
                        this.containerFilter(t))) {
                            ...
                            if (2 === e.sizeType) {
                                ...
                                if (s && s._html) {
                                    s = s._html;
                                    var a = s.type;
                                    "text" !== a && "image" !== a && "flash" !== a || (s.width = t.width,
                                    s.height = t.height)
                                }
                            }
                            var d = {
            ...
            launch: function() {
                ...
                try {
                    ...
                    for (var n in this.clbSlotArr)
                        "BAIDU_CLB_preloadSlots" === n ? this.data.defineOnce(n, i.proxy(this, this.reSet)) : this.data.defineOnce(n, i.proxy(this, this.getClbFillSlot));
                    this.updateApi()
                } catch (o) {}
            },
            reSet: function() {},
        ...
        i.define({
            ...
            render: function(t) {
                ...
                if (n && (this.expand.observer(t),
                this.bindEvent4Expand(t)),
                this.validate(t)) {
                    this.monitor.sendLog(t);
        ...
        i.define({
            ...
            processSlotInfo: function(t) {
                ...
                0 === a && "button" === g ? (v = "bottom",
                a = 40) : 0 === a && "couplet" === g && (v = "top",
                a = 150);
